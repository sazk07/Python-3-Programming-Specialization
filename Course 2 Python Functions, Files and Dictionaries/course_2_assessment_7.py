...
the first is required and should be an integer, the second 
is an optional parameter that can either be a number or a 
...
change in the definition of the function.
...
still have a required parameter, `intx`, and an optional 
parameter, `intz` with a defualt value of 5.
...
If the boolean parameter is True, the function should test 
...
to see if the first parameter is a key in the third parameter; 
if it is, return `True`, otherwise return `False`.
But if the second paramter is `False`, then the function should check 
