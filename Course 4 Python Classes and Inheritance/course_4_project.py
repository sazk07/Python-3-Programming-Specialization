...
and methods that WOFPlayer has, WOFHumanPlayer should have an additional method:
    .getMove(category, obscuredPhrase, guessed): Should ask the user to enter a 
    move (using input()) and return whatever string they entered.
...
that will be done via the game logic that we define in the next ActiveCode window.
"""
class WOFHumanPlayer(WOFPlayer):
    def getMove(self, category: str, obscuredPhrase: str, guessed: str) -> str:
        self.category=category
        self.obscuredPhrase=obscuredPhrase
        self.guessed=guessed
        print(f"{WOFPlayer.__str__(self)}\nCategory: {self.category}\nPhrase: {self.obscuredPhrase}\nGuessed: {self.guessed}")
        ask=input("Guess a letter, phrase, or type 'exit' or 'pass': ")
        if ask=="exit":
            exit()
        elif ask=="pass":
            pass
...
Finally, we’re going to define a class named WOFComputerPlayer, which should inherit 
from WOFPlayer (part A). This class is going to represent a computer player.
...
difficulty generally play “better”. There are many ways to implement this. We’ll do the following:
    If there aren’t any possible letters to choose (for example: if the last character is a 
    vowel but this player doesn’t have enough to guess a vowel), we’ll 'pass'
    Otherwise, semi-randomly decide whether to make a “good” move or a “bad” move on a given 
    turn (a higher difficulty should make it more likely for the player to make a “good” move)
...
Class variable
    .SORTED_FREQUENCIES: Should be set to 'ZQXJKVBPYGFWMUCLDRHSNIOATE', which is a list of 
    English characters sorted from least frequent ('Z') to most frequent ('E'). We’ll use this when trying to make a “good” move.
...
Methods
    .smartCoinFlip(): This method will help us decide semi-randomly whether to make a “good” or 
    “bad” move. A higher difficulty should make us more likely to make a “good” move. 
    Implement this by choosing a random number between 1 and 10 using random.randint(1, 10) 
    (see above) and returning False if that random number is greater than self.difficulty. 
    ...
    .getPossibleLetters(guessed): This method should return a list of letters that can be guessed.
            ...
            Additionally, if this player doesn’t have enough prize money to guess a 
            vowel (variable VOWEL_COST set to 250), then vowels (variable VOWELS set to 'AEIOU') should not be included
    .getMove(category, obscuredPhrase, guessed): Should return a valid move.
            ...
            If there aren’t any letters that can be guessed (this can happen if the only letters
            left to guess are vowels and the player doesn’t have enough for vowels), return 'pass'
            Use the .smartCoinFlip() method to decide whether to make a “good” or a “bad” move
                    If making a “good” move (.smartCoinFlip() returns True), then return the most 
                    frequent (highest index in .SORTED_FREQUENCIES) possible character
                    If making a “bad” move (.smartCoinFlip() returns False), then return a random 
                    character from the set of possible characters (use random.choice())
"""
class WOFComputerPlayer(WOFPlayer):
    ...
    def smartCoinFlip(self) -> bool:
        rand_number=random.randint(1,10)
        return False if rand_number>self.difficulty else True
    def getPossibleLetters(self,guessed: list) -> list:
        return [letter for letter in LETTERS if letter not in guessed and letter not in VOWELS and self.prizeMoney>VOWEL_COST]
    def getMove(self, category, obscurePhrase, guessed) -> str:
        possible=self.getPossibleLetters(guessed)
        if len(possible)==0:
            return "pass"
        else:
            if self.smartCoinFlip() is True:
                for letter in WOFComputerPlayer.SORTED_FREQUENCIES:
...
We have implemented most of the game logic. Start by carefully reading this code 
and double checking that it all makes sense. Then, paste your code from the previous code window in the correct places below.
...
from time import sleep
# Repeatedly asks the user for a number between min & max (inclusive)
def getNumberBetween(prompt: str, min: int, max: int):
    userinp = input(prompt) # ask the first time
    while True:
        try:
            n = int(userinp) # try casting to an integer
            if n < min:
                errmessage = f"Must be at least {min}"
            elif n > max:
                errmessage = f"Must be at most {max}"
            else:
                return n
        except ValueError: # The user didn't enter a number
...
# Example:
#     ("Artist & Song", "Whitney Houston's I Will Always Love You")
def getRandomCategoryAndPhrase() -> list | str:
    with open("phrases.json", 'r') as f:
        phrases = json.loads(f.read())
        category = random.choice(list(phrases.keys()))
        phrase   = random.choice(phrases[category])
...
#     phrase:  "GLACIER NATIONAL PARK"
#     returns> "_L___ER N____N_L P_RK"
def obscurePhrase(phrase: str, guessed: list) -> str:
    rv = ''
    for s in phrase:
        lambda rv: rv+'_' if s in LETTERS and s not in guessed else rv+s
    return rv
# Returns a string representing the current state of the game
def showBoard(category: str, obscuredPhrase: str, guessed: list) -> str:
    return f"""
Category: {category}
...
human_players = [WOFHumanPlayer(input(f"Enter the name for human player #{i+1}")) for i in range(num_human)]
...
winner = False
def requestPlayerMove(player: str, category: str, guessed: list) -> str:
...
while True:
    ...
    if wheelPrize['type'] == 'bankrupt':
        ...
        if move == 'EXIT': # leave the game
            ...
            if count > 0:
                lambda count: print(f"There is one {move}") if count==1 else print(f"There are {count} {move}'s")
