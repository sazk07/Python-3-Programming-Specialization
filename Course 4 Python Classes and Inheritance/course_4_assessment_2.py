"""
The class, Pokemon, is provided below and describes a Pokemon and 
its leveling and evolving characteristics. An instance of the class 
is one pokemon that you create.
...
that returns the string "[name of pokemon] knows a lot of different moves!". 
...
class Pokemon(object):
    ...
    def attack_up(self) -> int:
        self.attack=self.attack+self.attack_boost
        return self.attack

    def defense_up(self) -> int:
        self.defense=self.defense+self.defense_boost
        return self.defense

    def health_up(self) -> int:
...
instance to the variable p3. Then, use Grass_Pokemon methods to 
train the p3 Grass_Pokemon instance until it reaches at least level 10.
"""
class Pokemon(object):
    attack=12
    defense=10
...
subclasses. Call it opponent. It should return which type of pokemon the current 
type is weak and strong against, as a tuple.
...
class Pokemon():
    ...
    def opponent(self):
        weak_strong = dict({"Grass":("Fire","Water"),
        "Ghost":("Dark","Psychic"),
        "Fire":("Water","Grass"),
        "Flying":("Electric","Fighting")})
        for key in weak_strong:
            if key==self.p_type:
                self.weak=weak_strong[key][0]
                self.strong=weak_strong[key][1]
